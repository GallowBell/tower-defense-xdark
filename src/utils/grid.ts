...
export function gridToWorld(gridX: number, gridY: number): Vec2 {
  return {
    x:
      BALANCE.playfieldOffsetX +
      gridX * BALANCE.tileSize +
      BALANCE.tileSize / 2,
    y:
      BALANCE.playfieldOffsetY +
      gridY * BALANCE.tileSize +
      BALANCE.tileSize / 2,
  };
}
...
export function worldToGrid(worldX: number, worldY: number): Vec2 | null {
  const col = Math.floor(
    (worldX - BALANCE.playfieldOffsetX) / BALANCE.tileSize,
  );
  const row = Math.floor(
    (worldY - BALANCE.playfieldOffsetY) / BALANCE.tileSize,
  );
...
export function isInBounds(gridX: number, gridY: number): boolean {
  return (
    gridX >= 0 &&
    gridX < BALANCE.gridCols &&
    gridY >= 0 &&
    gridY < BALANCE.gridRows
  );
}
